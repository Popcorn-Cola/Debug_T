...
def traverse_backward(grad_fn):
    parameters = set()
    print('grad_fn:', grad_fn)
    if grad_fn is not None:
        for inp in grad_fn.next_functions:
            if inp[0] is not None:
                parameters.update(inp[0].parameters())
                traverse_backward(inp[0])
    return parameters
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('checkpts/grad_0.pt'))
        ...
        with tqdm(loader, total=len(train_dataset)//batch_size) as progress_bar:
            for batch_idx, batch in enumerate(progress_bar):
                ...
                if epoch ==10 or epoch ==1:
                    y_enc, y_dec, attn = model.forward(x, x_lengths, n_timesteps=params.timesteps, 
                                        temperature=1.5, stoc=False, spk=None, length_scale=0.91)
                    torch.save(y_enc , f'save/encoder_Debug_T_{epoch}.pt')
                    torch.save(model.state_dict(), f"save/{epoch}.pt")
                    prior_loss_fn = prior_loss.grad_fn

                    parameters = traverse_backward(prior_loss_fn)

                    print("Parameters connected to the Prior Loss tensor:")
                    for param in parameters:
                        print(param)
                    with open('P_G_Prior_Loss', 'w') as f:
                        f.write("Parameters connected to the Prior Loss tensor:\n")
                        for param in parameters:
                            f.write(str(param) + '\n')
                #####################################################################
